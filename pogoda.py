# ПРОГНОЗ ПОГДЫ v6.23b (пред релиз бота ).)
import pyowm
import telebot
...
def start_message(message):    
    ...
    bot.send_message(message.chat.id, "Приветствую тебя {0.first_name}!\nЯ - <b>{1.first_name}</b>, погода бот !".format(message.from_user, bot.get_me()),
        parse_mode='html', reply_markup=markup)
   

@bot.message_handler(commands=['textsend'])
def mess(message):
    if message.text == 'textsend' and ADMIN_CHAT_ID==[1123719362]:
...
def message_text(message):
    ...
    try: 
        ...
        if message.text[:10] == 'botinfo' or message.text[:10] == 'BOTINFO':
